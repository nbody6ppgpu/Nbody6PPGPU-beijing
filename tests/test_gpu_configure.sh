...
configure_case() {
  ...
  (cd "$root" && PATH="$tmp/base:$PATH" "$@" > "$tmp/$name.log" 2>&1)
...
if (cd "$root" && ./configure --disable-gpu --with-gpu-backend=hip >/dev/null 2>&1); then exit 1; fi
if (cd "$root" && ./configure --with-gpu-backend=bogus >/dev/null 2>&1); then exit 1; fi
if (cd "$root" && PATH="$tmp/base" ./configure --disable-mpi --enable-simd=no >/dev/null 2>&1); then exit 1; fi
