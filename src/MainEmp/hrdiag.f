...
* Kamlah 28.09.2021 - Tanikawa updates 27.09.2021
            !rg = ragbf(mt,lums(3),mt)
            rg = 10**(getRadiusRedPhase(mt,lums(3)))
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC		
         else
            rg = rgbf(mt,lums(3))
         endif
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
* Kamlah 28.09.2021 - Tanikawa updates 27.09.2021
               !ry = ragbf(mt,lum,zpars(2,tid))
               ry = 10**(getRadiusRedPhase(mt,lum))
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
               r  = MIN(r,ry)
            endif
*
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC				  
* Kamlah 28.09.2021 - Tanikawa updates 27.09.2021
!                  ry = ragbf(mt,lums(4),zpars(2, tid))
                  if(askInUseOrNot()
     &                 .and. askInScopeOfApplication(mass)) then
                     ry = 10**(getRadiusRedPhase(mt,lums(4)))
                  else
...
*
               ...
               r = rtms*(rx/rtms)**tau
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
* Kamlah 28.09.2021 - A. Tanikawa applies a patch for massive stars
* Tanikawa updates 27.09.2021
!               if(askInUseOrNot() 
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
* Kamlah 28.09.2021 - Tanikawa updates 27.09.2021
               !rg = ragbf(mt,lums(4),zpars(2,tid))
               rg = 10**(getRadiusRedPhase(mt,lums(4)))
...
*
* Tanikawa updates 27.09.2021
!            rx = ragbf(mt,lums(4),zpars(2, tid))
*
            rmin = min(rmin, rx)
            if(mass.le.mlp) then
               texp = log(mass/mlp)/log(zpars(3)/mlp)
...
!     &              .and. askInScopeOfApplication(mt)) then
     ...
     &              .and. askInScopeOfApplication(mass)) then
*
*     A. Tanikawa applies a patch for massive stars
                  !ry = ragbf(mt,lum,zpars(2,tid))
                  ry = 10**(getRadiusRedPhase(mt,lum)) 
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
                  r  = MIN(r,ry)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
* Kamlah 28.09.2021 - A. Tanikawa adds
               if(askInUseOrNot() 
     &              .and. askInScopeOfApplication(mass)) then
* Tanikawa updates 27.09.2021
                  !ry = ragbf(mt, lums(7),zpars(2))
                  ry = 10**(getRadiusRedPhase(mt,lums(7)))
*
               end if               
*
               rg = rg + tau*(ry - rg)
...
* Bug found by A. Tanikawa 25.10.2022
!               aj = MAX(aj,tm)
!               goto 90
*
            else
...
*    Level D        = 4: [Spera & Mapelli (2017)]  - future update             *
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
                  kw = 15
                  aj = 0.d0
                  mt = 0.d0
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
            ! mtc, mcmax might be negative, fixing it to 0.0 - Kamlah 27.08.21
            if (mtc.lt.0.0d0) then
              mc = 0.0d0
              mtc = max(mtc,0.0d0)
              mcmax = max(mcmax,0.0d0)
              kw = 15
              aj = 0.d0
              mt = 0.d0
              lum = 1.0d-10
...
*
     ...
     &           (mch/mc)**(2.d0/3.d0)-(mc/mch)**(2.d0/3.d0)))
      ...
      elseif(kw.eq.4)then
         ...
         CALL star(kw,mass,mt,tm,tn,tscls,lums,GB,zpars)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
*     A. Tanikawa modifies - Kamlah 15.09.2021
*         rc = rx
         if(askInUseOrNot() .and. askInScopeOfApplication(mass)) then
