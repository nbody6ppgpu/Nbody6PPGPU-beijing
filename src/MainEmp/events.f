...
!          IF (ABS(TM - TPHYS).GT.1.0.AND.TURN.LT.TURN2) GO TO 25
          ...
          if(askInUseOrNot()) then
             turn0 = turn2*0.5
             scopeflag0 = askInScopeOfApplication(turn0)
             scopeflag1 = askInScopeOfApplication(turn)
             if(scopeflag0.neqv.scopeflag1) then
                turnflag = .false.
                turn     = 0.5*(turn0+turn)
             endif
          endif
          IF (turnflag .and.
     &         ABS(TM - TPHYS).GT.1.0.AND.TURN.LT.TURN2) GO TO 25
*
      END IF
*
