...
*
      INCLUDE 'common6.h'
      REAL*8 TSCLS(20),LUMS(10),GB(10),TM,TN,mass
...
*       Base new time scale for changes in radius & mass on stellar type.
      if(kw.le.1)then
         dtm = pts1*tm
         dtr = tm - age
* Tanikawa changes here 15.10.2022 -- Intrinsic problem may come from uncertain values for kw=15.
!      elseif(kw.ge.10)then
      elseif(kw.ge.10 .and. kw.lt.15)then
*
C         dtm = 1.0d+02
         dtm = sqrt(0.1/lum)
         dtr = dtm
...
* Tanikawa's prescription
!      if(askInUseOrNot() .and. askInScopeOfApplication(m1)) then ! Kamlah 28.09.2021
      if(askInUseOrNot() .and. askInScopeOfApplication(M0)) then
         call calcTimestepAGBPhase(kw,age,M0,tn,pts2,dtm,dtr)
      endif
*
*
* Record radius.
