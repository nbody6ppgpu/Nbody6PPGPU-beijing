***
     ...
     &                 rzams,rtms,rg,menv,renv,k2e)
      ...
      real*8 k2e,menv,menvg,menvt,menvz,renv,renvg,renvt,renvz,rzr
...
* Kamlah 28.09.2021 - A. Tanikawa modifies 27.09.2021
      real*8 maxmass
      parameter(maxmass=3.0d2)
*     A. Tanikawa adds 08.09.2021 - Kamlah 15.09.2021
      real*8 logmtemp
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
*
...
* Kamlah 28.09.2021 - A. Tanikawa adds 27.09.2021
      logm = min(logm,log10(maxmass))
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
*
...
* A. Tanikawa adds 08.09.2021 - Kamlah 15.09.2021                              *
* Prevents negative moments of inertia for several times 100M* stars           *
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
*
*  Kamlah 28.09.2021 - A. Tanikawa changes 08.09.2021, and reverts 27.09.2021
      k2z = min(0.21d0,max(0.09d0-0.27d0*logm,0.037d0+0.033d0*logm))
      if(logm.gt.1.3d0) k2z = k2z - 0.055d0*(logm-1.3d0)**2
!      logmtemp = min(logm,log10(300.))
!      k2z = MIN(0.21d0,
!     &     MAX(0.09d0-0.27d0*logmtemp,0.037d0+0.033d0*logmtemp))
!      if(logmtemp.gt.1.3d0) k2z = k2z - 0.055d0*(logmtemp-1.3d0)**2
*
...
* CCCCCCCCC Changed below! - Albrecht, 15.09.2021 - modified 28.09.2021        *
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
         if (abs(lhei - lbgb).lt.1.0d-50) then
           x = 0.0d0
         else
...
* NOTE: Max on radius ratio added to stop large k2e (5/5/2009, JH)
*
            rzr = MAX(1.d0,rad/rzams)
            k2e = (k2z-E)*rzr**C + E*rzr**D
         elseif(kw.eq.7)then
...
* Kamlah 28.09.2021 - A. Tanikawa adds 27.09.2021
      if(k2e.le.0.d0 .and. kw.le.5) then
         write(0,*)'Fatal error in mrenv.f.',
     &        'Please contact A. Tankawa.',
     &        'kw,mass,mt,k2e=',kw,mass,mt,k2e
         stop
      endif
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
*
